    ...
    const handleClick = async () => {
        try {
            ...
            uploadTask.on('state_changed', (snapshot) => {
                ...
                updateMetadata(fileRef, newMetadata).then((metadata) => {
                    console.log('Metadata updated successfully', metadata);
                }).catch((error) => {
                    console.log("ERROR WITH METADATA", error.message)
                });
            ...
            <form className="form">
                ...
                <div className="dropdown">
                    ...
                    <div className="dropdown-content">
                        <button onClick={() => setGenre(genre.concat(" Educational"))}>Educational</button>
                        <button onClick={() => setGenre(genre.concat(" Sports"))}>Sports</button>
                        <button onClick={() => setGenre(genre.concat(" Beauty"))}>Beauty</button>
                        <button onClick={() => setGenre(genre.concat(" Fashion"))}>Fashion</button>
                        <button onClick={() => setGenre(genre.concat(" Comedy"))}>Comedy</button>
                        <button onClick={() => setGenre(genre.concat(" Family"))}>Family</button>
                    </div>
                </div>
                <br />
                <input type="file" onChange={(e) => setFile(e.target.files[0])} />
